...
void setup() {
  ...
  #if defined (__AVR_ATtiny85__)                        ////LED
    if (F_CPU == 12000000) clock_prescale_set(clock_div_1);
  #endif
  pixels.begin();                     
  pixels.show();
}
...
void loop() {
  ...
  {
    Serial.print(Direct_CapSensor[i]); 
    Serial.print(" ");  
  }
  Serial.println();
  */
...
uint16_t freq_to_waveleng(uint16_t f) 
{
  //return (uint16_t)(-0.1675 * f + 690.8875);
